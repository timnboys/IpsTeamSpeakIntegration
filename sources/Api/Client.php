...
class _Client extends \IPS\teamspeak\Api\AbstractConnection
{
    ...
    /**
     * Get all UUIDs that are registered on the TeamSpeak server.
     *
     * @return array
     */
    public function getAllUuids()
    {
        $clientDbList = \IPS\teamspeak\Api\Util::getReturnValue( $this->instance, $this->instance->clientDbList() );

        return $this->getUuidListFromClientDbList( $clientDbList );
    }
    ...
    /**
     * Only return UUIDs in a non-associative array.
     *
     * @param array $clientDbList
     * @return array
     */
    protected function getUuidListFromClientDbList( array $clientDbList )
    {
        $uuids = [];

        foreach ( $clientDbList as $client )
        {
            if ( mb_strlen( $client['client_unique_identifier'] ) === 28 )
            {
                $uuids[] = $client['client_unique_identifier'];
            }
        }
